...
class X4DataExtractor:
        ...
        for dev_data_path in possible_dev_paths:
            if dev_data_path.exists():
                ...
                if engines_dir.exists() and ships_dir.exists():
                    print(f"✅ Found development data at: {dev_data_path}")
    ...
    def _find_bundled_xrcattool(self) -> Optional[Path]:
        ...
        for tool_path in possible_paths:
            if tool_path.exists():
                print(f"✅ Found bundled XRCatTool at: {tool_path}")
                return tool_path
                
        print("⚠️ Bundled XRCatTool.exe not found")
        return None
    ...
    def _extract_with_xrcattool(self, tool_path: Path, cat_files: List[Path]) -> bool:
        ...
        try:
            ...
            print(f"🔧 Using XRCatTool: {tool_path}")
            print(f"📁 Extracting to: {self.data_dir.absolute()}")
            ...
            print(f"🌍 Detected language: {lang_name} (ID: {detected_lang_id})")
            print(f"📝 Will extract language file: {lang_file}")
            ...
            print("✅ XRCatTool extraction completed successfully")
            ...
            if engine_count > 0 and ships_found > 0:
                print(f"📊 Extracted {engine_count} engines, {ships_found} ships, {storage_count} storage modules")
                return True
            else:
                print(f"⚠️ Insufficient files extracted: {engine_count} engines, {ships_found} ships")
                return False
                
        except subprocess.CalledProcessError as e:
            print(f"❌ XRCatTool extraction failed: {e}")
            if e.stdout:
